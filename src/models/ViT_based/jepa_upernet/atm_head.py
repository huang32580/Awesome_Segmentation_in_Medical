...
# =========================================================================
# 去除了所有的 mmseg 依赖 (HEADS, BaseDecodeHead, accuracy 等)
# =========================================================================
...
# =========================================================================
# 将父类改为 nn.Module，接收原先由 kwargs 处理的参数 (如 in_channels)
# =========================================================================
class ATMHead(nn.Module):
    def __init__(
        ...
        # 处理输入通道数，如果传入的是列表，取第一个即可（因为各层 embed_dim 相同）
        self.in_channels = in_channels[0] if isinstance(in_channels, list) else in_channels
    ...
    def forward(self, inputs):
        ...
        # 抛弃 mmseg 的 losses 机制，直接将字典返回给我们的 Trainer 去接管
        return out
    ...
    # -----------------------------------------------------------
    # 注意：删除了原版中的 `losses` 和 `forward_with_loss`，
    # 因为它们属于 mmseg 架构下的函数调用，在我们的框架中永远不会被触发。
    # -----------------------------------------------------------